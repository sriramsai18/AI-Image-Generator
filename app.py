...
if not torch.cuda.is_available():
    try:
        ...
        total_ram = 8 * (1024**3)  # default/fallback to 8GB if SC parameters are unavailable (e.g. Windows)
...
def generate_image(prompt, negative_prompt, steps, guidance, width, height, seed):
    ...
    try:
        start = time.time()
        # Wrap inference in torch.inference_mode context to disable gradient tracking and improve speed
