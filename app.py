...
# Optimize memory layout for 2D convolutions (channels_last)
if hasattr(pipe, "unet") and pipe.unet is not None:
    pipe.unet.to(memory_format=torch.channels_last)
if hasattr(pipe, "vae") and pipe.vae is not None:
    pipe.vae.to(memory_format=torch.channels_last)

# Only enable attention slicing on CPU if RAM is under 4GB (< 4GB) to avoid ~230% CPU overhead
if not torch.cuda.is_available():
    try:
        total_ram = os.sysconf('SC_PAGE_SIZE') * os.sysconf('SC_PHYS_PAGES')
    except (AttributeError, ValueError):
        total_ram = 0
    if 0 < total_ram < 4 * 1024 * 1024 * 1024:
...
def generate_image(prompt, negative_prompt, steps, guidance, width, height, seed):
    ...
    try:
        start = time.time()
        # Disable autograd overhead during inference for maximum performance
