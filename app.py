...
# Performance optimization: channels_last memory format for faster 2D convolutions in UNet and VAE
if hasattr(pipe, "unet"):
    pipe.unet.to(memory_format=torch.channels_last)
if hasattr(pipe, "vae"):
    pipe.vae.to(memory_format=torch.channels_last)

# Only enable attention slicing on CPU if system memory is severely constrained (<4GB)
# to avoid significant CPU iteration slowdowns.
...
def generate_image(prompt, negative_prompt, steps, guidance, width, height, seed):
    ...
    try:
        start = time.time()
        # Performance optimization: torch.inference_mode disables autograd overhead during generation
