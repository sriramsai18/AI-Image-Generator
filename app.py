...
# Apply 'channels_last' memory format to UNet and VAE layers for optimal performance
if hasattr(pipe, "unet") and pipe.unet is not None:
    pipe.unet.to(memory_format=torch.channels_last)
if hasattr(pipe, "vae") and pipe.vae is not None:
    pipe.vae.to(memory_format=torch.channels_last)

# speed optimisation for CPU: only enable attention slicing if RAM is extremely constrained (<4GB)
if not torch.cuda.is_available():
    try:
        # Retrieve total system RAM in bytes on Unix systems
        total_ram = os.sysconf('SC_PAGE_SIZE') * os.sysconf('SC_PHYS_PAGES')
    except (AttributeError, ValueError, OSError):
        total_ram = 8 * (1024**3)  # default fallback to 8GB if not Unix or not available
...
def generate_image(prompt, negative_prompt, steps, guidance, width, height, seed):
    ...
    try:
        start = time.time()
        # Use torch.inference_mode() to disable view tracking and other overhead,
        # speeding up image generation significantly compared to no_grad.
