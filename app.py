...
# Optimizations: Channels-last memory layout on CUDA for fast 2D convolutions
...
def generate_image(prompt, negative_prompt, steps, guidance, width, height, seed):
    ...
    try:
        start = time.time()
        # Optimization: Use torch.inference_mode() to disable autograd tracking overhead & save memory
