...
# Optimize performance with channels_last memory format for UNet and VAE
if hasattr(pipe, "unet") and pipe.unet is not None:
    pipe.unet.to(memory_format=torch.channels_last)
if hasattr(pipe, "vae") and pipe.vae is not None:
    pipe.vae.to(memory_format=torch.channels_last)

# speed optimisation for CPU only if RAM is extremely constrained (<4GB)
if not torch.cuda.is_available():
    try:
        # Get system RAM in bytes on Unix systems
        page_size = os.sysconf('SC_PAGE_SIZE')
        phys_pages = os.sysconf('SC_PHYS_PAGES')
        total_ram = page_size * phys_pages
    except (AttributeError, ValueError):
        # Fallback to standard 8GB on unsupported platforms
        total_ram = 8 * 1024 * 1024 * 1024

    # Enable attention slicing only if RAM < 4GB
...
def generate_image(prompt, negative_prompt, steps, guidance, width, height, seed):
    ...
    try:
        start = time.time()
        # Use torch.inference_mode() for optimized forward pass performance
