...
# Speed optimization: apply channels_last memory format for faster 2D convolutions in UNet and VAE
if hasattr(pipe, "unet") and pipe.unet is not None:
    pipe.unet.to(memory_format=torch.channels_last)
if hasattr(pipe, "vae") and pipe.vae is not None:
    pipe.vae.to(memory_format=torch.channels_last)

# Speed optimization: Enable attention slicing on CPU only when RAM is constrained (<4GB).
# Attention slicing reduces memory usage but adds significant iteration overhead on systems with sufficient RAM.
if not torch.cuda.is_available():
    try:
        ram_gb = (os.sysconf("SC_PAGE_SIZE") * os.sysconf("SC_PHYS_PAGES")) / (1024 ** 3)
    except (AttributeError, ValueError, OSError):
        ram_gb = 8  # Fallback default if sysconf is unavailable
    if ram_gb < 4:
...
def generate_image(prompt, negative_prompt, steps, guidance, width, height, seed):
    ...
    try:
        start = time.time()
        # Speed optimization: use inference_mode to disable autograd & version tracking during inference
