...
# Performance optimization: Apply channels_last memory format for faster conv operations
if hasattr(pipe, "unet") and pipe.unet is not None:
    pipe.unet.to(memory_format=torch.channels_last)
if hasattr(pipe, "vae") and pipe.vae is not None:
    pipe.vae.to(memory_format=torch.channels_last)

# Performance optimization: Attention slicing reduces RAM usage at significant compute cost (~200%+ CPU slowdown).
# Enable attention slicing on CPU only when system RAM is severely constrained (< 4GB).
...
def generate_image(prompt, negative_prompt, steps, guidance, width, height, seed):
    ...
    try:
        start = time.time()
        # Performance optimization: torch.inference_mode disables autograd & version tracking for faster execution
